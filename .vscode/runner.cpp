...
OpRunner::OpRunner(OperatorDesc *opDesc, bool isDevice) : opDesc_(opDesc), isDevice_(isDevice)
{
...
bool OpRunner::RunOp()
{
    ...
    int execute_ = aclrtSynchronizeStream(stream);
    if (execute_ != ACL_SUCCESS) {
        ERROR_LOG("%d", execute_);
        (void)aclrtDestroyStream(stream);
...
template <typename T>
void DoPrintInt8Data(const T *data, size_t count, size_t elementsPerRow)
{
    for (size_t i = 0; i < count; ++i) {
        std::cout << std::setw(10) << int(data[i]);
        if (i % elementsPerRow == elementsPerRow - 1) {
            std::cout << std::endl;
        }
    }
...
void PrintData(const void *data, size_t count, aclDataType dataType, size_t elementsPerRow)
{
    ...
    switch (dataType) {
        case ACL_INT8:
            DoPrintInt8Data(reinterpret_cast<const int8_t *>(data), count, elementsPerRow);
            break;
        case ACL_INT32:
...
void OpRunner::PrintInput(size_t index, size_t numElementsPerRow)
{
    ...
    PrintData(hostInputs_[index],
        (GetInputElementCount(index) < 50 ? GetInputElementCount(index) : 50),
        aclGetTensorDescType(desc),
        numElementsPerRow);
}
